...
export default class Model {
  ...
  async FindAll(ownerUsername: string, projectName: string): Promise<IComment[] | undefined> {
    ...
    const result = await connection.execute<{ cursor: oracledb.ResultSet<any>; error: number }>(
      `BEGIN getComments(p_owner_username => :owner_username, p_project_name => :project_name, p_cursor => :cursor, p_error => :error); END;`,
      {

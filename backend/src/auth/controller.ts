...
export default class Controller {
  ...
  async SignUp(req: Request, res: Response, next: NextFunction) {
    try {
      const userData = await this._service.SignUp(req.body.username, req.body.password);
  ...
  async SignIn(req: Request, res: Response, next: NextFunction) {
    try {
      const userRecord = await this._service.SignIn(req.body.username, req.body.password);
